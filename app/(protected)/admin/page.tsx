...
import {
    ...
    CheckCircle2, XCircle, Search, ChevronDown, ChevronUp
} from 'lucide-react';
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                        <p className="text-lg font-bold text-foreground">{formatCurrency(u.balance)}</p>
                                        <p className="text-xs text-muted-foreground">{u.goals} goals</p>
                                    </div>
                                </div>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            ...
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className={`text-xs ${tx.status === 'completed' ? 'text-success' : 'text-destructive'}`}>
                                                    {tx.status}
                                                </span>
                                            </div>
                                        </div>
                    ...
                    <div className="block lg:hidden space-y-3">
                        {mockLedger.map((entry, i) => (
                            ...
                            >
                                ...
                                <p className="text-xs text-muted-foreground font-mono">{entry.transaction_id}</p>
                                <div className="flex justify-between pt-2 border-t border-border">
                                    <div>
                                        <p className="text-xs text-muted-foreground">Debit</p>
                                        <p className={`text-sm font-bold ${entry.debit > 0 ? 'text-destructive' : 'text-muted-foreground'}`}>
                                            {entry.debit > 0 ? formatCurrency(entry.debit) : '—'}
                                        </p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-xs text-muted-foreground">Credit</p>
                                        <p className={`text-sm font-bold ${entry.credit > 0 ? 'text-success' : 'text-muted-foreground'}`}>
            ...
            {tab === 'settings' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="md:col-span-2">
                        <Button variant="hero" onClick={handleSaveSettings} className="gap-2 w-full sm:w-auto">
