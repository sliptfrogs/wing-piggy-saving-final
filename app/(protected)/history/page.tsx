...
export default function TransactionHistory() {
  ...
  const filtered: UITransaction[] = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter(
        (tx) => tx.description.toLowerCase().includes(q) || tx.type.includes(q)
      );
    }
                      ...
                      type !== 'all' && (
                        <SelectItem key={type} value={type}>
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-5 h-5 rounded flex items-center justify-center ${cfg.color}`}
                            >
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3 sm:gap-4">
                              <div
                                className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center shrink-0 ${cfg.color}`}
                              >
                              ...
                              <div className="text-right shrink-0">
                                <p
                                  className={`text-sm sm:text-base font-bold tabular-nums ${tx.isCredit ? 'text-success' : 'text-foreground'}`}
                                >
                                  {tx.isCredit ? '+' : '−'}
