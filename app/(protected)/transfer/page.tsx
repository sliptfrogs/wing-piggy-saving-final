...
const tabs: {
  ...
  { type: 'p2p', icon: Users, label: 'P2P', desc: 'Send to another user' },
  {
    ...
    icon: Heart,
    label: 'Contribute',
    desc: "Contribute to someone's goal",
  },
];
...
export default function Transfer() {
  ...
  const [selectedPiggyAccountNumber, setSelectedPiggyAccountNumber] =
    useState('');
  ...
  const accountTypeParam =
    type === 'p2p' ? 'MAIN' : type === 'contribute' ? 'PIGGY' : undefined;
  const {
    data: accountData,
  ...
  const { mutate: transferContribute, isPending: isContributePending } =
    useTransferContribute();
  const { mutate: transferOwnPiggy, isPending: isOwnPiggyPending } =
    useTransferOwnPiggy();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (type === 'p2p') {
      ...
      transferOwnPiggy(
        {
          recipient_account_number: selectedPiggyAccountNumber,
          amount: amt,
          notes,
        },
        {
    ...
    <div className="px-4 sm:px-6 xl:px-8 py-5 sm:py-6 xl:py-8 max-w-[1400px] mx-auto space-y-5 sm:space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Move money to goals or other users
        </p>
      </div>
        ...
        <div className="xl:col-span-2 space-y-5 sm:space-y-6">
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            {tabs.map(({ type: t, icon: Icon, label, desc }) => (
              ...
              >
                <div
                  className={`w-8 h-8 rounded-lg flex items-center justify-center mb-1 ${type === t ? 'gradient-primary text-primary-foreground' : 'bg-secondary text-muted-foreground'}`}
                >
                  <Icon className="w-4 h-4" />
                </div>
                <span
                  className={`text-sm font-semibold ${type === t ? 'text-primary' : 'text-foreground'}`}
                >
                  {label}
                </span>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <Button
                      ...
                      disabled={
                        accountLoading || !recipientAccountNumber.trim()
                      }
                ...
                >
                  ...
                  {piggyLoading ? (
                    ...
                    <p className="text-muted-foreground text-sm">
                      No piggy goals found. Create one first.
                    </p>
                  ) : (
                    <Select
                      value={selectedPiggy}
            ...
            <div className="glass rounded-2xl p-4 sm:p-5 space-y-2">
              ...
              {amount && !isNaN(amt) && (
                <p
                  className={`text-xs font-medium ${amt > mainBalance ? 'text-destructive' : 'text-muted-foreground'}`}
                >
                  {amt > mainBalance
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-semibold text-foreground">
                  {tabs.find((t) => t.type === type)?.label || '—'}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">To</span>
                <span className="text-sm font-semibold text-foreground break-all text-right max-w-[60%]">
                  {getRecipientDisplay()}
                </span>
              </div>
              {type === 'contribute' && notes && (
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm text-foreground break-all text-right max-w-[60%]">
                    {notes}
                  </span>
                </div>
              )}
